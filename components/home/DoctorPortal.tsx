...
const DoctorPortal = () => {
  ...
  const handleDemoSubmit = (values: any) => {
    setIsSubmitting(true);
    
    // Simulate form submission
    setTimeout(() => {
      setIsSubmitting(false);
      // In a real app, we would redirect to the dashboard on successful login
      // Here we'll just redirect to the login page which will check auth state
            ...
            <div className="space-y-4 mb-6">
              {[1, 2, 3, 4].map((item) => (
                <div key={item} className="flex items-start">
                  ...
                  <div>
                    <h4 className="font-medium">{t(`doctorPortal.features.feature${item}.title`)}</h4>
                    <p className="text-text-medium">{t(`doctorPortal.features.feature${item}.description`)}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="mt-6 pt-6 border-t border-neutral-200">
              ...
              <div className="flex justify-center">
                <Link href="/espace-medecins/login">
                  <a className="text-primary hover:underline text-sm group">
                    {t('doctorPortal.preview.demo')} <Check className="inline-block h-4 w-4 ml-1 transition-transform group-hover:translate-x-1" />
                  </a>
                </Link>
              </div>
            </div>
